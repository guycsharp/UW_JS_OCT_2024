...
const cardsWorthTen = (cards) => {
  // const valDeck = cards.map((card) => {
  //   if (card.val==10) {
  //     // card; // why is this card object is not added in valDeck? // because I forgot to add 'return'
  //     // console.log(card); // this prints the correct cards
  //     return card;
  //   } else {
  //     continue;
  //   }
  // });
  const val10Deck = cards.filter((card)=>card.val==10);
  const displayDeck = val10Deck.map((card)=>card.displayVal);
  return displayDeck;
};
